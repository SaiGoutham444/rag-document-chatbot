...
class TestParseCitations:
    ...
    def test_extracts_all_citations(self, citation_enforcer, sample_citation_answer):
        """parse_citations() must find all citation tags."""
        citations = citation_enforcer.parse_citations(sample_citation_answer)
        assert len(citations) == 3

    def test_extracts_correct_filename(self, citation_enforcer, sample_citation_answer):
        """Each CitationRef must have correct filename."""
        citations = citation_enforcer.parse_citations(sample_citation_answer)
        for c in citations:
            assert c.filename == "test_report.pdf"

    def test_extracts_correct_page(self, citation_enforcer, sample_citation_answer):
    ...
    def test_extracts_chunk_id(self, citation_enforcer, sample_citation_answer):
...
class TestValidateCitations:
    ...
    def test_hallucinated_citation_flagged(self, citation_enforcer, sample_chunks):
        """Citations with fake chunk_ids must be marked invalid."""
        answer = (
            "Some claim " "[SOURCE: test.pdf | PAGE: 1 | CHUNK: completely_fake_id_xyz]"
        )
        citations = citation_enforcer.parse_citations(answer)
        report = citation_enforcer.validate_citations(citations, sample_chunks)
    ...
    def test_coverage_score_calculation(self, citation_enforcer, sample_chunks):
    ...
    def test_no_citations_gives_zero_score(self, citation_enforcer, sample_chunks):
...
class TestEnforceCitations:
    ...
    def test_cannot_find_response_is_valid(self, citation_enforcer, sample_chunks):
    ...
    def test_valid_answer_returns_is_valid_true(self, citation_enforcer, sample_chunks):
